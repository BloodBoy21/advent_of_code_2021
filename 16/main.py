...
class Packet:
    ...
    lengthTypeID = (
        -1
    )  # 0 or 1.  0 = 15 bits represent # bits, 1 = 11 bits represent # packets
    rawSubData = ""
    subPackets = []
    value = -1
        ...
        while subBitsProcessed < numBits:
            ...
            if newPacket.type == Packet.PacketType.literal:
                ...
                if newPacket.lengthTypeID == 0:
                    ...
                    numSubPackets = int(binData[i : i + 11], 2)
                    i += 11
                    sp, bitsProcessed = getSubPackets(
                        binData[i:], numPackets=numSubPackets
                    )
                    newPacket.subPackets.extend(sp)
                    i += bitsProcessed
...
def part1(lines):
    ...
    binData = binData.zfill(
        len(lines[0] * 4)
    )  # Leading zeros wlil get truncated by bin().  Add them back here
    packet = packetsFromBinary(
        binData
    )  # there is always a single outer packet, not multiple (although it probably contains many sub packets)
    return packet
